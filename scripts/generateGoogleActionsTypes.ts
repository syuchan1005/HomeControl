...
const fetchDataFromNetworkOrCache = async (url) => {
  await fs.mkdir('scripts/cache').catch(() => {
  }/* ignored */);
...
type TraitObject = {
  ...
  name: string;
  description: string;
  attributes: TypeObjectWithKey;
  states: TypeObjectWithKey;
  commands: {
    [key: string]: TypeObject;
  };
};
...
const _genTypeValueText = (
  value: TypeObject,
  enableIoTs: boolean = false,
  baseIndent = 0,
  indentStep = 2,
) => {
  let valueText;
...
const genTypeText = (
  obj: TypeObject | TypeObjectWithKey,
  enableIoTs: boolean = false,
  baseIndent = 0,
  indentStep = 2,
) => {
  if (obj.type && typeof obj.type === 'string') {
    return _genTypeValueText(obj as TypeObject, enableIoTs);
  }
  const data = Object.entries(obj as TypeObjectWithKey)
    .map(([key, value]) => `${' '.repeat(baseIndent + indentStep)
    }${normalizeKey(key)}: ${_genTypeValueText(value as TypeObject, enableIoTs, baseIndent + indentStep, indentStep)}`);
  return enableIoTs
    ? `t.type({${data.length === 0 ? '' : '\n'}${data.join(',\n')}${data.length === 0 ? '' : ',\n'}${' '.repeat(baseIndent)}})`
    : `{${data.length === 0 ? '' : '\n'}${data.join(';\n')}${data.length === 0 ? '' : ';\n'}${' '.repeat(baseIndent)}}`;
};
      ...
      .map((row) => {
        ...
        if (row.children.length === 2) {
          ...
          definitionText = definitionText
            .trim()
            .toLowerCase();
          let type = getType(definitionText);
          if (!type) return undefined;
          if (type === 'unknown'
            && nestElement
...
const fetchTraitTypes = async () => {
  ...
  for (const trait of traits) {
    console.log(`[Processing] ${trait.type}`);
          ...
          case 'device-commands': {
            ...
            while (elements.length > i + commandId && elements[i + commandId].tagName !== 'H2') {
              if (elements[i + commandId].tagName === 'H3') {
                type = elements[i + commandId].textContent.trim();
...
const genTraitTypeInformationText = (traits: Array<TraitObject>) => {
...
const genCommandTypeInformationText = (traits: Array<TraitObject>) => {
  const registeredKeys = [];
  const content = traits
    .flatMap((trait) => Object.entries(trait.commands))
    .map(([name, command]) => {
      ...
      return `  ${key}: {
    type: '${name}',
    typeObject: ${genTypeText(command, true, 4)}
  },`;
    }).filter((t) => !!t).join('\n');
