<template>
  <v-list-tile class="height-auto-tile">
    ...
    <div v-else-if="type === 'ColorSetting'">
      <v-text-field
        hide-details
                    ...
                    <v-card style="padding: 10px;">
                      ...
                      setting.setting_values.push({});
                        ...
                        <v-list-tile v-for="(langValue, i) in setting.setting_values" :key="i"
                          ...
                          <v-combobox v-model="langValue.setting_synonym" label="setting_synonym"
                                      chips multiple append-icon="" style="margin-left: 10px"/>
    ...
    <div v-else-if="type === 'Scene'">
      <div style="display: flex">
        <v-spacer />
        no fields
      </div>
    </div>
    <div v-else-if="type === 'StartStop'">
      <v-text-field
        hide-details
        label="getCommand (return true or false)"
        placeholder="ex. 'switch get'"
    ...
    <div v-else-if="type === 'TemperatureControl'">
      <v-text-field
        hide-details
        label="minTemp"
        placeholder="ex. 'switch set %v'"
    ...
    <div v-else-if="type === 'TemperatureSetting'">
    ...
    <div v-else-if="type === 'Toggles'">
      <v-text-field
        hide-details
        label="setCommand"
        placeholder="ex. 'switch set %toggle %value'"
        v-model="info.setCommand"/>
      <v-text-field
        hide-details
        label="getCommand (return true or false)"
      ...
      <v-list>
        <v-subheader>
          ...
          <v-spacer/>
          <v-btn icon @click="() => {
            if (!info.toggles) $set(info, 'toggles', []);
            info.toggles.push({});
          }">
            <v-icon>fa fa-plus</v-icon>
          </v-btn>
        </v-subheader>
        <v-list-tile v-for="(toggle, i) in info.toggles" :key="i"
                     class="height-auto-tile">
          <v-card>
            <div style="padding-left: 10px;">
              <div style="display: flex">
                <v-text-field
                  ...
                  label="ToggleName"
                  v-model="toggle.name"/>
                <v-btn icon @click="info.toggles.splice(i, 1)">
                  <v-icon small>fa fa-trash</v-icon>
                </v-btn>
              </div>
              <v-card style="margin: 5px;">
                <v-list>
                  <v-subheader>
                    ...
                    <v-spacer/>
                    <v-btn icon @click="() => {
                      if (!toggle.name_values) $set(toggle, 'name_values', []);
                      toggle.name_values.push({});
                    }">
                      <v-icon>fa fa-plus</v-icon>
                    </v-btn>
                  </v-subheader>
                  <v-list-tile v-for="(lang, i) in toggle.name_values" :key="i"
                               class="height-auto-tile">
                    <v-text-field v-model="lang.lang" label="lang" style="max-width: 4rem;"/>
                    <v-combobox v-model="lang.name_synonym" label="speed_synonym"
                                chips multiple append-icon="" style="margin-left: 10px"/>

                    <v-btn icon @click="toggle.name_values.splice(i, 1)">
                      <v-icon small>fa fa-trash</v-icon>
                    </v-btn>
                  </v-list-tile>
                </v-list>
